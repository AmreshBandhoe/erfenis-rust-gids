...
"De Erfeniswijzer" ("The Legacy Guide") is a Dutch website about inheritance and estate planning ("nalatenschap en erfenis"). The project has two language variants:

- A Dutch (NL) site, which is the default build.
- An English (EN) site, built as a separate deployment.

Both variants are built from the same codebase; the language is selected at build time (see section 6) and can also be toggled by the visitor in the browser (see section 8).
...
│   ├── lib/
│   │   ├── i18n.tsx          # language context/provider (NL/EN)
│   │   ├── translations.ts   # translation/content object (~1150 lines) for both languages
...
├── wrangler.en.toml              # Cloudflare deployment config for the EN site
├── vite.config.ts
...
"scripts": {
  ...
  "build:pages:en": "cross-env VITE_DEFAULT_LANG=en vite build && node -e \"require('fs').copyFileSync('dist/server/server.js','dist/client/_worker.js')\"",
...
- **`build:pages`** — runs `vite build`, then copies `dist/server/server.js` to `dist/client/_worker.js`. Cloudflare Pages auto-detects `_worker.js` in the output directory as the SSR handler. This produces the Dutch (NL) build, using the default language.
- **`build:pages:en`** — identical to `build:pages`, but sets the `VITE_DEFAULT_LANG=en` environment variable (via `cross-env`) before building, producing an English-default build for the separate EN deployment.
...
The project deploys to **Cloudflare Workers/Pages** as two separate deployments — one per language.
...
`wrangler.en.toml` (EN site):
```toml
name = "erfenis-rust-gids-en"
main = "dist/server/server.js"
compatibility_date = "2024-09-23"
compatibility_flags = ["nodejs_compat"]
...
- Neither file defines `routes`, a custom domain, a `zone_id`, or any KV/D1/R2 bindings — domain/route attachment is not present in these config files and would need to be configured separately (e.g. in the Cloudflare dashboard).
- The EN deployment (`erfenis-rust-gids-en`) needs to be built with `npm run build:pages:en` specifically before being deployed under `wrangler.en.toml`, since that build sets `VITE_DEFAULT_LANG=en`.
...
|---|---|---|---|
| `VITE_DEFAULT_LANG` | Build-time (`import.meta.env`) | `src/lib/i18n.tsx`, `src/components/Header.tsx` | Set to `"en"` by `build:pages:en` (via `cross-env`); determines the default site language (`nl` if unset, `en` if set) and whether the language-toggle button is shown in the header. |
...
No `.env`, `.env.example`, or `.dev.vars` files exist anywhere in the repository. `.gitignore` lists `.dev.vars` and `*.local` as ignored patterns, but no such files are present or committed. `src/lib/config.server.ts` also contains commented-out placeholder fields (`databaseUrl`, `stripeSecretKey`) showing an intended pattern for future secrets, but no database or Stripe integration is currently wired up.
...
- `src/lib/translations.ts` exports a single object keyed by language (`"nl" | "en"`), containing nested strings and content objects for navigation, footer, per-page copy, and knowledge-base articles for both languages.
- `src/lib/i18n.tsx` defines a `LanguageProvider` (React Context) that determines the default language from the `VITE_DEFAULT_LANG` build-time variable, and on the client overrides that default if a language was previously stored in `localStorage` (key: `"lang"`). It exposes `useT()` (current language's translation object) and `useLang()` (`{ lang, setLang, t }`).
...
- In practice, the NL and EN sites are two separate builds/deployments of the same codebase (`build:pages` vs. `build:pages:en`, deployed via `wrangler.toml` vs. `wrangler.en.toml`), each with a client-side toggle for switching language within whichever build the visitor is on.
...
- `wrangler.en.toml` is present on disk but is currently untracked in git (not committed).
