...
import { ShieldCheck, HandHeart, MessagesSquare, CheckCircle2 } from "lucide-react";
import { ContentHero } from "@/components/ContentHero";
import { CtaSection } from "@/components/CtaSection";
import { useT } from "@/lib/i18n";
import heroImg from "@/assets/hulp-hero.jpg";

export const Route = createFileRoute("/executeurschap")({
  head: () => ({
    meta: [
      { title: "Executeurschap — De Erfeniswijzer" },
      {
        name: "description",
        content:
          "Een onafhankelijke executeur nodig? Wij begeleiden de afwikkeling van de nalatenschap zorgvuldig en volgens de wensen van de overledene.",
      },
      { property: "og:title", content: "Executeurschap — De Erfeniswijzer" },
      {
        property: "og:description",
        content:
          "Onafhankelijk executeurschap: de afwikkeling van de nalatenschap in vertrouwde handen.",
      },
      { property: "og:image", content: heroImg },
      { property: "twitter:image", content: heroImg },
    ],
  }),
  component: Executeurschap,
});

const topicIcons = [ShieldCheck, HandHeart, MessagesSquare];

function Executeurschap() {
  const t = useT();
  const h = t.executeurschap;

  return (
    <>
      <ContentHero
        image={heroImg}
        imageAlt="Adviseur bespreekt rustig documenten met nabestaanden aan tafel"
